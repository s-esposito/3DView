...
/* Where a panel scrolls, so none can outgrow the window (every host sets
   html,body{overflow:hidden}, which would put the overflow out of reach). It has to be
   this grandchild: clamping .viewer-clip would fight the 1fr row the collapse animates,
   and a scroll box on .viewer-panel would clip the "+" menu. The cap is halved-viewport
   so that BOTH panels plus the column's fixed chrome (two headers, gap, top offset,
   version caption ~= 140px) always fit, and only then capped at 40vh. border-box or the
   padding sits on top of the cap; overflow-x hidden or the knob/thumb animations raise a
   spurious horizontal bar. */
.viewer-body{padding:4px 12px 14px;box-sizing:border-box;max-height:var(--viewer-panel-max);overflow:hidden auto}
...
/* Both of the UI's scrollers: a thin bar that tones with the theme, so it reads as
   part of the glass rather than as the host's default gutter. */
.viewer-body::-webkit-scrollbar,.viewer-modal-list::-webkit-scrollbar{width:8px}
.viewer-body::-webkit-scrollbar-track,.viewer-modal-list::-webkit-scrollbar-track{background:transparent}
.viewer-body::-webkit-scrollbar-thumb,.viewer-modal-list::-webkit-scrollbar-thumb{border:2px solid transparent;background:color-mix(in srgb,var(--vscode-foreground,#ddd) 22%,transparent);background-clip:content-box;border-radius:99em}
.viewer-body::-webkit-scrollbar-thumb:hover,.viewer-modal-list::-webkit-scrollbar-thumb:hover{background:color-mix(in srgb,var(--vscode-foreground,#ddd) 38%,transparent);background-clip:content-box}
